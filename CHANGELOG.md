...
- **Launch banner with version + freshness.** Every `cwc` launch now prints a one-line `[cwc] image: <tag> (<state>)` banner before the session summary. States: `latest stable` (`:latest` or matches GitHub's most recent release), `stable, latest`, `stable, outdated -- X.Y.Z is available`, `preview build, latest` (moving `:preview` tag), `preview build of X.Y.Z, pinned` (SHA-pinned `:X.Y.Z.<sha>` tag), `custom tag`. The freshness check hits `api.github.com/repos/.../releases/latest` and caches the result at `~/.cwc/version-cache.json` for 24h, so most launches don't make a network call. Set `CWC_SKIP_VERSION_CHECK=1` to disable the check (CI / offline). Dev sessions print their own `[cwc dev] running with ...` banner instead.
- **Preview builds use SHA-suffixed Docker tags + a moving `:preview` pointer.** Each `preview/<x.y.z>` push at commit `<sha>` produces SHA-pinned Docker tags `:x.y.z.<sha>` (and `-ltsc2019` / `-ltsc2022` variants), preserved on Docker Hub, plus moving `:preview` / `:preview-ltsc2019` / `:preview-ltsc2022` tags overwritten on each push. The GitHub release for previews uses the literal git tag `preview` (single moving release that softprops upserts; older preview release content is replaced on each push). Multiple preview builds for the same `x.y.z` no longer collide. Stable releases continue to use plain `vX.Y.Z` tags. `install.ps1` gains `-Ref preview` to install from the moving preview release.
- **Install channel persisted to `~/.cwc/config.json`.** `install.ps1` writes a new `channel` field (`stable` or `preview`) based on the `-Ref` argument: `-Ref preview` → `channel: preview`, anything else → `channel: stable`. `cwc` reads this on launch and picks the default Docker tag accordingly — `channel: stable` → `:latest`, `channel: preview` → `:preview`. `$env:CWC_IMAGE` continues to override. Reinstalling from the other channel rewrites only the `channel` field; `host_denylist` and `defaults` are preserved. This means a one-time preview install keeps you on `:preview` automatically across shells and reboots, without needing to remember to set `CWC_IMAGE`.
...
- **Preview install command in README and `docs/commands.md` no longer 404s.** The previous `$args = @('-Ref','preview'); irm ... | iex` pattern was a silent no-op: `iex` runs the script content in the current scope, but PowerShell's parameter binding doesn't read the parent scope's `$args` automatic variable to fill a script's `param()` block. The downloaded `install.ps1` always ran with `$Ref = 'latest'` and tried to fetch from `releases/latest/download/...` — which 404s whenever there's no published non-prerelease release. Replaced with `& ([scriptblock]::Create((irm ...))) -Ref preview`, which actually forwards the parameter. The same fix applies to the `-Ref v1.2.3` and `-Ref main` examples in `docs/commands.md`.
- **Preview release body now points at the preview install URL.** `scripts/build-release-body.sh` was emitting `irm releases/latest/download/install.ps1 | iex` in the Quickstart for *every* release, including previews -- so the preview release page told users to install from the stable channel (and would 404 if no stable was published). Preview releases now show the working `& ([scriptblock]::Create((irm releases/download/preview/install.ps1))) -Ref preview` form, plus a follow-up note that `cwc` defaults to `:preview` images afterwards via the channel field.
- **Preview release body now lists the rolling `:preview` Docker tag.** The image table only showed SHA-pinned tags (`:0.1.1.a27125`), even though the workflow already pushed `:preview` / `:preview-ltsc2019` / `:preview-ltsc2022` on each preview build. Now the table includes both -- SHA-pinned for reproducibility, rolling `:preview` for "whatever the team is currently testing".
- **CI host-test filter widened to match the local fast suite.** `.github/workflows/build-and-release.yml` ran `tests/run.ps1 -Filter 'denylist|trust'` (14 tests). Now runs `dev|denylist|trust|project-isolation|setup` (29 tests, same ~2s) -- matches the suite documented in `CLAUDE.md`, so breakage in `cwc dev`, `cwc setup`, or per-project isolation is caught in CI instead of only locally.
